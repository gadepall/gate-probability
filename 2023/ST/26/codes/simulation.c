...
int main() {
    ...
    for (int i = 0; i < num_samples; i++) {
        ...
        theoretical_cdf_values[i] = cdf(random_var); // Calculate and store the theoretical CDF value
    }
    ...
    FILE *random_vars_file = fopen("random_vars.dat", "w");
    ...
    FILE *cdf_file = fopen("cdf_values.dat", "w");
    if (cdf_file == NULL) {
        perror("Error opening file");
        return 1;
    }
    ...
    FILE *theoretical_cdf_file = fopen("theoretical_cdf_values.dat", "w");
